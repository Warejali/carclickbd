...
const LeftSidebarDropdown: React.FC<IMenuItem> = ({
  ...
  return (
    <div className="transition-all duration-300 ease-in-out w-full">
      ...
      <button
        onClick={onToggle}
        className="flex items-center justify-between w-full px-3 py-2 rounded-md hover:bg-green-500 hover:text-white transition-colors duration-300"
      >
        ...
        <span
          className={`${
            !isSidebarOpen
              ? "hidden"
              : "text-md transition-transform duration-500"
          } ${isOpen ? "rotate-180" : ""}`}
        >
          <RiArrowDownSLine />
        </span>
